// StorySpark Production Server v2
...
async function falRun(endpoint, input){
  ...
  console.log(`[FAL] Submitting to ${endpoint} with input:`, JSON.stringify(input, null, 2));
  
  // Submit
  const payload = {input};
  const fb=Buffer.from(JSON.stringify(payload));
  
  console.log(`[FAL] Full JSON payload:`, JSON.stringify(payload, null, 2));
  console.log(`[FAL] Payload buffer length: ${fb.length} bytes`);
  ...
  console.log(`[FAL] Submit response status: ${sub.status}`);
  
  if(sub.status!==200){
    const errorBody = sub.body.toString();
    console.error(`[FAL] Submit error:`, errorBody);
    throw new Error(`Fal submit ${sub.status}: ${errorBody.substring(0,400)}`);
  }
  
  const submitResponse = JSON.parse(sub.body.toString());
  console.log(`[FAL] Submit response:`, JSON.stringify(submitResponse, null, 2));
  
  const {request_id} = submitResponse;
  if(!request_id) throw new Error('No request_id in fal.ai submit response');
  ...
  for(let i=0;i<120;i++){
    ...
    const s=JSON.parse(poll.body.toString());
    
    console.log(`[FAL] Poll ${i+1}: status = ${s.status}`);
    ...
    if(s.status==='COMPLETED'){
      ...
      const fullResponse = JSON.parse(result.body.toString());
      console.log(`[FAL] Final result:`, JSON.stringify(fullResponse, null, 2));
      
      // Return the data field if it exists, otherwise return the full response
      return fullResponse.data || fullResponse;
    }
    if(s.status==='FAILED'){
      console.error(`[FAL] Request failed:`, s);
      throw new Error(`Fal failed: ${s.error||JSON.stringify(s)}`);
    }
  }
...
[{"page":1,"text":"story text","sceneDesc":"vivid 15-word scene description for AI illustration","emoji":"🌟","mood":"wonder"}]`;
    ...
    if(pathname==='/api/illustrate'&&req.method==='POST'){
      ...
      if(withFace&&photoUrl){
        // Use flux-pulid for face-consistent generation
        const params = {
          prompt:`${PIXAR}, ${prompt}`,
        ...
        console.log('[ILLUSTRATE] Calling falRun with params:', JSON.stringify(params, null, 2));
        result=await falRun('fal-ai/flux-pulid', params);
      }else{
        // Use flux-schnell for generic generation
        const params = {
          prompt:`${PIXAR}, ${prompt}`,
          image_size:'landscape_4_3',
        ...
        console.log('[ILLUSTRATE] Calling falRun with params:', JSON.stringify(params, null, 2));
        result=await falRun('fal-ai/flux/schnell', params);
      }

      // DEBUG: Log the full response structure
      console.log('=== FAL.AI RESPONSE DEBUG ===');
      ...
      if(result?.images?.[0]?.url){
        ...
        imgUrl=result.data.images[0].url;
      }else if(result?.data?.url){
        imgUrl=result.data.url;
      }else{
        console.error('❌ Could not find image URL in any known format');
        // TEMPORARY: Send the full response structure to frontend for debugging
        json(res,{
...
server.listen(PORT,()=>{
  console.log(`\n  ╔═══════════════════════════════════════════╗`);
  console.log(`  ║   StorySpark Production Server v2         ║`);
