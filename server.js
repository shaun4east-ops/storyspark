...
async function falRun(endpoint, input){
  ...
  // Submit
  const fb=Buffer.from(JSON.stringify({input}));
    ...
    if(pathname==='/api/illustrate'&&req.method==='POST'){
      ...
      if(withFace&&photoUrl){
        ...
        result=await falRun('fal-ai/flux-pulid',{
          main_face_image_url:photoUrl,  // FIXED: was reference_image_url
          prompt:`${PIXAR}, ${prompt}`,
          negative_prompt:`${NEG}${negPrompt?', '+negPrompt:''}`,
          num_images:1,
          image_size:{width:768,height:1024},  // FIXED: portrait_4_3 is not valid anymore
